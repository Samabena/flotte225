...
**Priority:** Could Have
**Story:**
> As a system administrator, I want the system to automatically send a fleet summary payload to a configured webhook URL every 24 hours, so that external tools receive regular fleet updates without manual intervention.
...
- [ ] Given the webhook fires, when I check the payload, then it includes the owner identity, period covered, alert summary, and driver activity delta since last send.
...
**Acceptance Criteria:**
- [ ] Given I am on the export page, when I select a dataset type, a format (PDF or Excel), and optionally a date range, then the file is generated and downloaded to my device.
...
- [ ] Given I am on the Starter plan, when I attempt to access the export page, then I see a locked-state UI with an upgrade prompt.
...
- [ ] Given I am on the Pro plan and have used 5 reports this month, when I try to generate another, then the button is disabled with a monthly limit message.
...
**Acceptance Criteria:**
- [ ] Given I am on the reports page, when I enable the schedule and select Weekly or Monthly, then the schedule is saved and active.
- [ ] Given a schedule is active, when the scheduled time arrives, then the system automatically generates and emails the report.
- [ ] Given I toggle the schedule off, when I save, then no further automated reports are sent until re-enabled.
- [ ] Given I am on the Pro plan, when I try to enable a scheduled report, then the option is locked (Business plan required).
...
- [ ] Given WhatsApp API credentials are not configured in the environment, when I save my number, then the feature is silently disabled.
...
**Priority:** Should Have
**Story:**
> As a fleet owner, I want to receive WhatsApp messages for critical alerts and detected performance anomalies, so that urgent issues reach me instantly even when I'm not logged into the app.

**Acceptance Criteria:**
- [ ] Given a vehicle has a critical alert (insurance expired or inspection expired), when the notification engine runs, then a WhatsApp message is sent to my configured number.
- [ ] Given a vehicle is flagged with a performance anomaly, when the notification engine runs, then a WhatsApp message is sent with the anomaly details.
- [ ] Given no critical alerts or anomalies exist, when the notification engine runs, then no WhatsApp message is sent.
- [ ] Given a daily summary is due, when it triggers, then the WhatsApp message contains the alert summary and driver activity delta.
...
- [ ] Given I confirm the deletion, when it completes, then the user and all their associated data are permanently removed.
...
**Priority:** Must Have
**Story:**
> As a super admin, I want to view any owner's vehicle list and fleet summary, so that I can assist with support requests or audit data without logging in as that owner.
...
- [ ] Given I select a new plan and save, when the change is applied, then the owner's feature access updates immediately.
...
- [ ] Given multiple plans are in use, when I view the plan distribution section, then I see how many owners are on each plan.
...
- [ ] Given the public registration form, when any user submits it, then only OWNER or DRIVER roles are selectable.
...
**Acceptance Criteria:**
- [ ] Given a Starter owner has 2 vehicles, when they call the create-vehicle API endpoint, then the response is a 403 with an upgrade message.
...
**Acceptance Criteria:**
- [ ] Given I am on the Starter plan, when I navigate to export, AI reports, anomaly detection, or WhatsApp pages, then the feature area is shown in a locked state with which plan unlocks it.
- [ ] Given I click "Upgrade", when the modal opens, then I see a plan comparison table (Starter / Pro / Business) with features and pricing.
- [ ] Given simulation mode is active, when I click upgrade, then I am shown a message to contact the administrator.
...
**Priority:** Should Have
**Story:**
> As a fleet owner, I want to see my current subscription plan and usage indicators on my profile page, so that I know where I stand before hitting any limits.
...
- [ ] Given I am approaching a limit, when I view my profile, then a visual indicator highlights I am at the limit.
- [ ] Given simulation mode is active, when I click "Upgrade", then I see a contact-admin message instead of a payment form.
...
*20 / 20 story points*
*Goal: Super admin can manage all users and plans. Owners see their plan, usage, and upgrade prompts. Dashboard visualizations are complete.*
