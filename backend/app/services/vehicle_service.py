...
def _check_plan_vehicle_limit(db: Session, owner_id: int) -> None:
    sub = (
    ...
    if not sub:
        return  # no subscription — allow (edge case)
    ...
    if plan and plan.max_vehicles is not None:
        ...
        if active_count >= plan.max_vehicles:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail=f"Limite atteinte : votre abonnement {plan.name} autorise {plan.max_vehicles} véhicules actifs maximum",
            )


def _get_driver_or_400(db: Session, owner_id: int, driver_id: int) -> User:
    """Return the driver only if they exist and have the DRIVER role."""
    driver = db.get(User, driver_id)
    if not driver or driver.role != "DRIVER":
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Chauffeur introuvable ou rôle invalide",
        )
    return driver
...
def create_vehicle(db: Session, owner_id: int, data: VehicleCreate) -> Vehicle:
    _check_plan_vehicle_limit(db, owner_id)
...
def update_vehicle(db: Session, owner_id: int, vehicle_id: int, data: VehicleUpdate) -> Vehicle:
    vehicle = _get_vehicle_or_404(db, owner_id, vehicle_id)

    if vehicle.status == "archived":
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Impossible de modifier un véhicule archivé")

    updates = data.model_dump(exclude_none=True)

    if "license_plate" in updates and updates["license_plate"] != vehicle.license_plate:
        if db.query(Vehicle).filter(Vehicle.license_plate == updates["license_plate"]).first():
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Ce numéro de plaque existe déjà",
            )
...
def pause_vehicle(db: Session, owner_id: int, vehicle_id: int) -> Vehicle:
    ...
    if vehicle.status != "active":
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Seul un véhicule actif peut être mis en pause")
...
def resume_vehicle(db: Session, owner_id: int, vehicle_id: int) -> Vehicle:
    ...
    if vehicle.status != "paused":
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Seul un véhicule en pause peut être relancé")
...
def archive_vehicle(db: Session, owner_id: int, vehicle_id: int) -> Vehicle:
    vehicle = _get_vehicle_or_404(db, owner_id, vehicle_id)
    if vehicle.status == "archived":
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Ce véhicule est déjà archivé")

    # Reset any active driver currently driving this vehicle
    active_driver = db.query(User).filter(
        User.active_vehicle_id == vehicle_id,
        User.driving_status == True,
    ).first()
    if active_driver:
...
def restore_vehicle(db: Session, owner_id: int, vehicle_id: int) -> Vehicle:
    vehicle = _get_vehicle_or_404(db, owner_id, vehicle_id)
    if vehicle.status != "archived":
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Seul un véhicule archivé peut être restauré")
...
def _do_assign_driver(db: Session, owner_id: int, vehicle_id: int, driver_id: int) -> None:
    vehicle = _get_vehicle_or_404(db, owner_id, vehicle_id)
    if vehicle.status == "archived":
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Impossible d'assigner un chauffeur à un véhicule archivé")

    driver = _get_driver_or_400(db, owner_id, driver_id)

    existing = db.query(VehicleDriver).filter(
        VehicleDriver.vehicle_id == vehicle_id,
        VehicleDriver.driver_id == driver_id,
    ).first()
    if existing:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Ce chauffeur est déjà assigné à ce véhicule")
...
def remove_driver(db: Session, owner_id: int, vehicle_id: int, driver_id: int) -> None:
    ...
    if not assignment:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Ce chauffeur n'est pas assigné à ce véhicule")
...
def get_vehicle_drivers(db: Session, owner_id: int, vehicle_id: int) -> list[User]:
    _get_vehicle_or_404(db, owner_id, vehicle_id)
    assignments = db.query(VehicleDriver).filter(VehicleDriver.vehicle_id == vehicle_id).all()
...
def get_driver_vehicles(db: Session, driver_id: int) -> list[Vehicle]:
    assignments = db.query(VehicleDriver).filter(VehicleDriver.driver_id == driver_id).all()
...
def activate_driver(db: Session, driver: User, vehicle_id: int) -> User:
    ...
    if not assignment:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Vous n'êtes pas assigné à ce véhicule")

    vehicle = db.get(Vehicle, vehicle_id)
    if not vehicle or vehicle.status != "active":
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Ce véhicule n'est pas disponible")
