...
def update_vehicle(
    ...
    return _ok(
        data=VehicleResponse.model_validate(vehicle), message="Véhicule mis à jour"
    )
...
def pause_vehicle(
    ...
    return _ok(
        data=VehicleResponse.model_validate(vehicle), message="Véhicule mis en pause"
    )
...
def restore_vehicle(
    ...
    return _ok(
        data=VehicleResponse.model_validate(vehicle), message="Véhicule restauré"
    )
...
@router.post(
    "/{vehicle_id}/drivers", status_code=status.HTTP_201_CREATED, response_model=None
)
def assign_driver(
    vehicle_id: int,
