...
def _create_driver_in_db(db, owner_id, username="testdriver.maint"):
...
def owner_token(client, db):
    _seed_plans(db)
    return _register_and_verify(client, db, "owner@maint.ci")
...
def driver_token(client, db, owner_token):
    ...
    _create_driver_in_db(db, owner_id, username="testdriver.maint")
    res = client.post(
        "/api/v1/auth/login",
        json={"identifier": "testdriver.maint", "password": "Password1"},
    )
    return res.json()["access_token"]
...
def test_update_maintenance_partial(client, db, owner_headers):
    ...
    client.put(
        f"/api/v1/vehicles/{vid}/maintenance",
        json={"insurance_expiry": expiry},
        headers=owner_headers,
    )
    res = client.put(
        f"/api/v1/vehicles/{vid}/maintenance",
        json={"last_oil_change_km": 5000},
        headers=owner_headers,
    )
    data = res.json()["data"]
...
def test_critical_alert_for_expired_inspection(client, db, owner_headers):
    ...
    client.put(
        f"/api/v1/vehicles/{vid}/maintenance",
        json={"inspection_expiry": expired},
        headers=owner_headers,
    )
...
def test_no_oil_alert_without_last_oil_change_km(
...
def test_warning_oil_alert_between_400_and_500(
    client, db, owner_headers, driver_token, driver_headers
):
...
def test_critical_oil_alert_above_500(
    client, db, owner_headers, driver_token, driver_headers
):
...
def test_oil_alert_clears_after_update(
    client, db, owner_headers, driver_token, driver_headers
):
    ...
    _add_fuel_entry(
        db, vid, driver_id, 10600, 40, 24000
    )  # 600 km since oil change → critical

    # Reset oil change
    client.put(
        f"/api/v1/vehicles/{vid}/maintenance",
        json={"last_oil_change_km": 10600},
        headers=owner_headers,
    )
...
def test_no_anomaly_with_fewer_than_2_entries(
    client, db, owner_headers, driver_token, driver_headers
):
...
def test_consumption_anomaly_detected(
    client, db, owner_headers, driver_token, driver_headers
):
...
def test_no_anomaly_within_20_percent(
    client, db, owner_headers, driver_token, driver_headers
):
...
def test_no_cost_spike_without_last_month_data(
    client, db, owner_headers, driver_token, driver_headers
):
...
def test_no_cost_spike_within_30_percent(
    client, db, owner_headers, driver_token, driver_headers
):
...
def test_custom_expense_type_is_accepted(client, db, owner_headers):
    """A type outside the suggestion list (user picks "Autre") is stored verbatim."""
    vid = _create_vehicle(client, owner_headers, "MX-CUSTOM-CI")
    res = client.post(
        f"/api/v1/vehicles/{vid}/maintenance-expenses",
        json=_expense_payload(type="Climatisation"),
        headers=owner_headers,
    )
    assert res.status_code == 200
    assert res.json()["data"]["type"] == "Climatisation"


def test_empty_expense_type_is_rejected(client, db, owner_headers):
    vid = _create_vehicle(client, owner_headers, "MX-EMPTY-CI")
    res = client.post(
        f"/api/v1/vehicles/{vid}/maintenance-expenses",
        json=_expense_payload(type="   "),
        headers=owner_headers,
    )
