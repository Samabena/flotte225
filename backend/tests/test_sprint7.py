...
class TestExport:
    def test_pro_owner_can_export_fuel_as_excel(self, client, db):
        ...
        res = client.post(
            "/api/v1/export?format=excel&type=fuel", headers=_headers(owner_token)
        )
        assert res.status_code == 200
        assert "spreadsheetml" in res.headers["content-type"]
        assert (
            res.headers["content-disposition"]
            == 'attachment; filename="carburant.xlsx"'
        )
    ...
    def test_pro_owner_can_export_fuel_as_pdf(self, client, db):
        ...
        res = client.post(
            "/api/v1/export?format=pdf&type=fuel", headers=_headers(owner_token)
        )
        assert res.status_code == 200
        assert res.headers["content-type"] == "application/pdf"
        assert (
            res.headers["content-disposition"] == 'attachment; filename="carburant.pdf"'
        )
    ...
    def test_pro_owner_can_export_maintenance_as_excel(self, client, db):
        ...
        res = client.post(
            "/api/v1/export?format=excel&type=maintenance",
    ...
    def test_invalid_format_returns_400(self, client, db):
        ...
        res = client.post(
            "/api/v1/export?format=csv&type=fuel", headers=_headers(owner_token)
        )
    ...
    def test_invalid_type_returns_400(self, client, db):
        ...
        res = client.post(
            "/api/v1/export?format=excel&type=vehicles", headers=_headers(owner_token)
        )
        assert res.status_code == 400

    def test_driver_cannot_export(self, client, db):
        _seed_plans(db)
        owner_token = _register_and_verify(client, db, "owner31a_helper@test.ci")
        driver_token, _ = _make_driver_token(client, db, owner_token, "driver31a")

        res = client.post(
            "/api/v1/export?format=excel&type=fuel", headers=_headers(driver_token)
        )
        assert res.status_code == 403
...
class TestWhatsAppConfig:
    ...
    def test_driver_cannot_configure_whatsapp(self, client, db):
        _seed_plans(db)
        owner_token = _register_and_verify(client, db, "owner34a_helper@test.ci")
        driver_token, _ = _make_driver_token(client, db, owner_token, "driver34a")
...
class TestWhatsAppAlerts:
    ...
    def test_send_fleet_alerts_sends_for_critical(self):
        ...
        critical_alert = AlertResponse(
            vehicle_id=1,
            vehicle_name="Camion 01",
            license_plate="AA-001-CI",
        ...
        with patch(
            ...
            result = send_fleet_alerts_to_owner(
                "+22507000000", "Jean Dupont", [critical_alert]
            )
    ...
    def test_whatsapp_api_call_uses_correct_payload(self):
        ...
        with patch("app.services.whatsapp_service.settings") as mock_settings, patch(
            "app.services.whatsapp_service.httpx.post", return_value=mock_response
        ) as mock_post:
            mock_settings.WHATSAPP_API_URL = (
...
class TestAdminAnalytics:
    def test_admin_gets_platform_stats(self, client, db):
        ...
        owner_token = _register_and_verify(client, db, "owner41a@test.ci")
        owner_id = _get_user_id(owner_token)
        _create_driver_in_db(db, owner_id, "driver41a")
    ...
    def test_analytics_counts_are_correct(self, client, db):
        ...
        before = client.get(
            "/api/v1/admin/analytics", headers=_headers(admin_token)
        ).json()["data"]
    ...
    def test_analytics_total_spend_sums_all_fuel_entries(self, client, db):
        ...
        db.add(
            FuelEntry(
                ...
                date=date.today(),
                odometer_km=100,
                quantity_litres="30.00",
                amount_fcfa="25000.00",
            )
        )
        db.commit()
