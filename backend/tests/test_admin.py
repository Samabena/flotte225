...
def _create_admin(client, db, email):
    ...
    db.commit()
    res = client.post(
        "/api/v1/auth/login", json={"identifier": email, "password": "AdminPass1"}
    )
    return res.json()["access_token"]
...
class TestListUsers:
    def test_admin_lists_all_users(self, client, db):
        ...
        owner_token = _register_and_verify(client, db, "owner36a@test.ci")
        owner_id = _get_user_id(owner_token)
        _create_driver_in_db(db, owner_id, "driver36a")

        res = client.get("/api/v1/admin/users", headers=_headers(admin_token))
        assert res.status_code == 200
        assert res.json()["success"] is True
        emails = [u["email"] for u in res.json()["data"]]
        roles = [u["role"] for u in res.json()["data"]]
    ...
    def test_search_by_email(self, client, db):
        ...
        _register_and_verify(client, db, "findme36@test.ci")

        res = client.get(
            "/api/v1/admin/users?q=findme36", headers=_headers(admin_token)
        )
        data = res.json()["data"]
        assert len(data) == 1
    ...
    def test_filter_by_role(self, client, db):
        ...
        owner_token = _register_and_verify(client, db, "owner36d@test.ci")
        owner_id = _get_user_id(owner_token)
        _create_driver_in_db(db, owner_id, "driver36d")

        res = client.get(
            "/api/v1/admin/users?role=DRIVER", headers=_headers(admin_token)
        )
        data = res.json()["data"]
    ...
    def test_search_no_match_returns_empty(self, client, db):
        ...
        res = client.get(
            "/api/v1/admin/users?q=zzznomatch", headers=_headers(admin_token)
        )
...
class TestSuspendReactivate:
    def test_admin_can_suspend_owner(self, client, db):
        ...
        res = client.patch(
            f"/api/v1/admin/users/{owner_id}/suspend", headers=_headers(admin_token)
        )
        assert res.status_code == 200
    ...
    def test_suspended_user_cannot_login(self, client, db):
        ...
        res = client.post(
            "/api/v1/auth/login",
            json={"identifier": "owner37b@test.ci", "password": "Password1"},
        )
    ...
    def test_admin_can_reactivate_user(self, client, db):
        ...
        res = client.patch(
            f"/api/v1/admin/users/{owner_id}/reactivate", headers=_headers(admin_token)
        )
        assert res.status_code == 200
    ...
    def test_cannot_suspend_super_admin(self, client, db):
        ...
        res = client.patch(
            f"/api/v1/admin/users/{admin2_id}/suspend", headers=_headers(admin_token)
        )
        assert res.status_code == 403
    ...
    def test_suspend_unknown_user_returns_404(self, client, db):
        ...
        res = client.patch(
            "/api/v1/admin/users/99999/suspend", headers=_headers(admin_token)
        )
...
class TestDeleteUser:
    def test_admin_can_delete_driver(self, client, db):
        ...
        owner_token = _register_and_verify(client, db, "owner38a_helper@test.ci")
        owner_id = _get_user_id(owner_token)
        driver = _create_driver_in_db(db, owner_id, "driver38a")
        driver_id = driver.id

        res = client.delete(
            f"/api/v1/admin/users/{driver_id}", headers=_headers(admin_token)
        )
    ...
    def test_admin_can_delete_owner_and_fleet(self, client, db):
        ...
        res = client.delete(
            f"/api/v1/admin/users/{owner_id}", headers=_headers(admin_token)
        )
        assert res.status_code == 204
    ...
    def test_cannot_delete_super_admin(self, client, db):
        ...
        res = client.delete(
            f"/api/v1/admin/users/{admin2_id}", headers=_headers(admin_token)
        )
        assert res.status_code == 403
    ...
    def test_admin_cannot_delete_self(self, client, db):
        ...
        res = client.delete(
            f"/api/v1/admin/users/{admin_id}", headers=_headers(admin_token)
        )
        assert res.status_code == 403
...
class TestAdminViewFleet:
    def test_admin_sees_owner_vehicles(self, client, db):
        ...
        res = client.get(
            f"/api/v1/admin/users/{owner_id}/fleet", headers=_headers(admin_token)
        )
        assert res.status_code == 200
        data = res.json()["data"]
    ...
    def test_fleet_returns_400_for_non_owner(self, client, db):
        ...
        owner_token = _register_and_verify(client, db, "owner39c_helper@test.ci")
        owner_id = _get_user_id(owner_token)
        driver = _create_driver_in_db(db, owner_id, "driver39c")

        res = client.get(
            f"/api/v1/admin/users/{driver.id}/fleet", headers=_headers(admin_token)
        )
    ...
    def test_non_admin_cannot_view_fleet(self, client, db):
        ...
        res = client.get(
            f"/api/v1/admin/users/{owner_id}/fleet", headers=_headers(owner_token)
        )
        assert res.status_code == 403
...
class TestAssignPlan:
    def test_admin_can_upgrade_owner_to_pro(self, client, db):
        ...
        plan_res = client.get(
            "/api/v1/subscription/my-plan", headers=_headers(owner_token)
        )
    ...
    def test_assign_plan_to_non_owner_returns_400(self, client, db):
        ...
        owner_token = _register_and_verify(client, db, "owner40d_helper@test.ci")
        owner_id = _get_user_id(owner_token)
        driver = _create_driver_in_db(db, owner_id, "driver40d")

        res = client.put(
            f"/api/v1/admin/users/{driver.id}/plan",
            json={"plan_name": "pro"},
            headers=_headers(admin_token),
        )
        assert res.status_code == 400
...
class TestMyPlan:
    ...
    def test_usage_counts_assigned_drivers(self, client, db):
        ...
        owner_id = _get_user_id(owner_token)
        driver = _create_driver_in_db(db, owner_id, "driver46c")
    ...
    def test_driver_cannot_access_plan_endpoint(self, client, db):
        _seed_plans(db)
        owner_token = _register_and_verify(client, db, "owner46d_helper@test.ci")
        driver_token, _ = _make_driver_token(client, db, owner_token, "driver46d")
