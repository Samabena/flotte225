...
def _create_driver_in_db(db, owner_id, username="testdriver.fuel"):
...
def owner_token(client, db):
    _seed_plans(db)
    return _register_and_verify(client, db, "owner@fuel.ci")
...
def driver_token(client, db, owner_token):
    ...
    _create_driver_in_db(db, owner_id, username="testdriver.fuel")
    res = client.post(
        "/api/v1/auth/login",
        json={"identifier": "testdriver.fuel", "password": "Password1"},
    )
    return res.json()["access_token"]
...
def test_submit_fuel_entry_success(
...
def test_submit_rejected_when_wrong_vehicle(
    ...
    driver_id = _get_driver_id(client, db, driver_token)
    client.post(
        f"/api/v1/vehicles/{vid1}/drivers",
        json={"driver_id": driver_id},
        headers=owner_headers,
    )
    client.post(
        f"/api/v1/vehicles/{vid2}/drivers",
        json={"driver_id": driver_id},
        headers=owner_headers,
    )
    client.post(
        "/api/v1/driver/activate", json={"vehicle_id": vid1}, headers=driver_headers
    )
...
def test_submit_rejected_when_odometer_not_greater(
    client, db, owner_headers, driver_token, driver_headers
):
...
def test_list_driver_entries_returns_last_10(
    client, db, owner_headers, driver_token, driver_headers
):
...
def test_edit_fuel_entry_success(
    client, db, owner_headers, driver_token, driver_headers
):
    ...
    res = client.patch(
        f"/api/v1/fuel/{entry_id}",
        json={"amount_fcfa": "30000.00"},
        headers=driver_headers,
    )
    assert res.status_code == 200
...
def test_edit_fuel_entry_locked_after_24h(
    client, db, owner_headers, driver_token, driver_headers
):
...
def test_edit_rejects_bad_odometer(
    client, db, owner_headers, driver_token, driver_headers
):
    ...
    res = client.patch(
        f"/api/v1/fuel/{entry_id}", json={"odometer_km": 10000}, headers=driver_headers
    )
    assert res.status_code == 422
...
def test_delete_fuel_entry_success(
    client, db, owner_headers, driver_token, driver_headers
):
...
def test_delete_fuel_entry_locked_after_24h(
    client, db, owner_headers, driver_token, driver_headers
):
...
def test_owner_views_fleet_entries(
    client, db, owner_headers, driver_token, driver_headers
):
...
def test_owner_cannot_see_other_owners_entries(
    client, db, owner_headers, driver_token, driver_headers
):
    """A second owner registers, their fleet is empty."""
    def _seed_plans(): pass  # already seeded in fixture
...
def test_owner_fleet_entries_read_only(
    client, db, owner_headers, driver_token, driver_headers
):
    ...
    assert (
        client.patch(
            f"/api/v1/fuel/{entry_id}",
            json={"amount_fcfa": "1.00"},
            headers=owner_headers,
        ).status_code
        == 403
    )
    assert (
        client.delete(f"/api/v1/fuel/{entry_id}", headers=owner_headers).status_code
        == 403
    )
...
def test_owner_views_activity_log(
    client, db, owner_headers, driver_token, driver_headers
):
...
def test_activity_log_ordered_newest_first(
    client, db, owner_headers, driver_token, driver_headers
):
